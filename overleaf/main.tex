...
liderados pela extrativa mineral. A posição no \emph{ranking} de intensidade em setores de base
é, portanto, \textbf{condicional à classificação adotada}; o ranking reestimado sem Alimentos
não é reportado aqui, e a afirmação deve ser lida sob esse rótulo. Já a liderança da extrativa
mineral e o baixo emprego relativo do setor dominante independem dele.}
...
(Figura~\ref{fig:vocacao}). Os quocientes locacionais desta seção derivam da regionalização
não censitária de 2015, sem correção do tipo FLQ, e por isso indicam a \emph{ordem} da
especialização de cada território, não o seu nível exato (\S\ref{sec:limites}).
A Região Metropolitana (Grande Vitória) concentra 62\% da
...
das demais microrregiões cairia \textbf{13,0\%} sob a injeção da demanda final
\emph{intra-estadual} (Apêndice~\ref{ap:hem}) --- a periferia capixaba depende do núcleo
...
externa do ES é praticamente nula. Registre-se que essa igualdade é uma \emph{identidade
algébrica} entre as duas formas de escrever o mesmo termo, e não um cálculo independente: o
valor reportado é o \emph{feedback} da eq.~\eqref{eq:feedback}, computado pela inversa
particionada, e a decomposição de Miyazawa é apresentada aqui pela leitura que oferece --- a
separação entre amplificação interna e externa ---, não como segunda estimativa do número.
...
índices --- calculado como desvio-padrão \emph{populacional} sobre a média dos 26 setores ---
mede o grau de integração da economia.

Uma ressalva de interpretação acompanha o índice para frente. A inversa de Ghosh é o núcleo
do modelo \emph{oferta-dirigido}, cuja leitura causal --- um choque de oferta primária
propagando-se a jusante com coeficientes de alocação fixos --- é contestada na literatura por
implicar substituição perfeita entre insumos. Aqui $G$ é usada no sentido restrito e
consensual: como medida da \emph{posição} do setor na cadeia, isto é, de quanto da produção
dos demais setores passa por ele a montante. Os índices para frente deste artigo, e a
classificação de setores-chave que deles resulta, devem ser lidos como \emph{descrição de
posição}, não como previsão do impacto de um choque de oferta.
