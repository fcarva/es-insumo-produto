...
colunas = os 10 setores (de 35) que definem vocação. LQ>1 (vermelho) = especializado.
Padrão editorial: sem título embutido (legenda LaTeX assume), vírgula decimal,
PNG 300 dpi + PDF vetorial. Le: outputs/micro_LQ.csv.
"""
import os, sys
...
fig.subplots_adjust(left=0.145, right=0.94, top=0.97, bottom=0.24)
...
ax.grid(False)   # sem grid maior: as linhas cortavam as anotações no centro das células
for i in range(M.shape[0]):
    for j in range(M.shape[1]):
        v = M[i, j]
        if v >= 1.3:
            ax.text(j, i, estilo.br(v, 1), ha="center", va="center", fontsize=7.6,
                    color=estilo.WHITE if v > 3 else estilo.INK, fontweight="bold", zorder=4)
# separadores finos de célula (bordas, não cruzam o conteúdo)
ax.set_xticks(np.arange(-.5, len(setores), 1), minor=True)
ax.set_yticks(np.arange(-.5, len(ordem_reg), 1), minor=True)
ax.grid(which="minor", color=estilo.WHITE, linewidth=0.8)
ax.tick_params(which="minor", length=0)
ax.set_facecolor(estilo.WHITE)   # base branca p/ as células de LQ baixo (sem dado = branco)
cb = fig.colorbar(im, ax=ax, fraction=0.025, pad=0.01)
cb.set_ticks([0, 2, 4, 6]); cb.set_ticklabels(["0", "2", "4", "≥6"])
cb.set_label("quociente locacional (LQ)", fontsize=8)
estilo.salvar(fig, OUT, "fig_micro_vocacao")
