...
serviços e administração pública. A decomposição da produção pela demanda final
quantifica quem puxa essa economia: \textbf{61,7\%} da produção capixaba é induzida por
demanda \emph{externa} ao estado (exportações e demanda do restante do Brasil), mas essa
mesma demanda externa responde por apenas 47,5\% do emprego --- o motor está fora, o
trabalho fica desproporcionalmente com o mercado interno. A estrutura é \emph{dual}: a extração (petróleo e
...
Essa base é, ademais, \emph{aberta}: 24,9\% do multiplicador vaza para fora do estado e o
\emph{spillover} escoa concentrado ao núcleo São Paulo--Rio (52,5\%) com \emph{feedback} de
apenas 0,32\% --- mas um modelo nulo sobre as 27 unidades da federação mostra que essa
mecânica é a prevista para o porte do estado (o ES não é \emph{outlier} em nenhuma métrica
de decomposição): o que a abertura transporta de específico é a composição. Essa lógica
reproduz-se, em \emph{escalas aninhadas}, entre as microrregiões.
...
têxtil no Centro-Oeste, agropecuária nas serranas) --- e os multiplicadores territoriais
confirmam o desenho centro-periferia: a metrópole retém 90,9\% de seu multiplicador de
produção dentro do próprio território, contra 66,2\% no Litoral Sul extrativo.
...
O gênero operacionaliza, em linguagem de matriz, as duas lentes fundadoras da teoria do
desenvolvimento regional: a \emph{base de exportação} --- o crescimento regional comandado
pela demanda externa \citep{north1955,tiebout1956} --- e os \emph{polos de crescimento} ---
indústrias motrizes que irradiam encadeamentos sobre a estrutura ao redor \citep{perroux1955}.
...
Pará \citep{sessofilho2010para}, entre outros. Para o Espírito Santo, há contribuições \emph{pontuais}:
...
interno que emprega. É a fotografia insumo-produto da \emph{base de exportação} de
\citet{north1955} --- a demanda externa comanda o crescimento da produção ---, com a
ressalva de \citet{tiebout1956} reaparecendo pelo lado do emprego: são as atividades
\emph{residentes}, voltadas ao mercado interno, que carregam a ocupação. O canal tieboutiano
...
São polos na acepção de \citet{perroux1955}: unidades motrizes de grande porte cujo
dinamismo se transmite à economia adjacente pelos encadeamentos --- exatamente o que a
medida de ligações puras quantifica. O grau de integração (coeficiente de variação das
...
inaugurou esse desenho para os estados brasileiros e já documentava transbordamentos
intensos, e pouco recíprocos, ao restante do país.
...
vaza-da-periferia configura, pois, uma relação \emph{centro-periferia} que se reproduz em
\emph{escalas aninhadas}: a tradição regional brasileira lê o ES como periferia do núcleo
industrial paulista \citep{cano1985}, e aqui o mesmo padrão reaparece \emph{dentro} do estado ---
a periferia está para o ES como o ES está para o Sudeste; a leitura por escalas encaixadas de
núcleos e periferias remonta a \citet{friedmann1966}. É a mesma assinatura de \emph{enclave} que
a concentração territorial já indicava (\S\ref{sec:territorio}), agora pelo lado do fluxo.
A pequenez do \emph{feedback} é, em boa medida, regularidade de porte \citep{miller1966} ---
e o modelo nulo (Apêndice~\ref{ap:nulo}) a quantifica: regredidas as três métricas de
...
Base exportadora que comanda a produção \citep{north1955}, polos motrizes que concentram o
encadeamento \citep{perroux1955} e hierarquia centro-periferia que captura o valor
\citep{cano1985,friedmann1966} são, aqui, três descrições do mesmo dado.
