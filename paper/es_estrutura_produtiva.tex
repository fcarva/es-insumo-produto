...
%  18_trajetoria_n68.py (C2), 20/24 (C3 território) e 25_decomposicao_fd.py (C4).
% =====================================================================
\documentclass[11pt,a4paper]{article}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage[english,brazilian]{babel} % brazilian ativo (hifenização PT); english p/ o abstract
...
insumo-produto, na tradição brasileira de análise de economias estaduais, combinando a
matriz inter-regional ES\,$\times$\,restante do Brasil de 2008 (26 setores, com emprego),
...
os setores líderes --- mineração, petróleo, siderurgia e celulose --- concentram
encadeamento (as maiores ligações puras), mas geram pouco emprego e renda diretos; a
extração opera como \emph{enclave}, a transformação como setor-chave. A decomposição pela
demanda final quantifica o descolamento: a demanda externa ao estado induz \textbf{61,7\%}
da produção, mas apenas 47,5\% do emprego. Um benchmark com as 27 unidades da federação
...
retém 90,9\% de seu multiplicador contrasta com periferias especializadas que vazam; na
abertura, o encadeamento escoa ao núcleo São Paulo--Rio com \emph{feedback} quase nulo ---
mecânica prevista pelo porte do estado: a especificidade capixaba é composicional.
...
\noindent\textbf{\emph{Productive structure of Espírito Santo, Brazil: an input-output
analysis}}

\vspace{0.5em}
\noindent This paper characterizes the productive structure of the Brazilian state of
Espírito Santo (ES) through the input-output lens, in the Brazilian tradition of
state-level structural analysis, combining the 2008 interregional ES--rest-of-Brazil
matrix (26 sectors, with employment), the national 68-sector series (2010--2021), and the
interregional system of the state's ten planning microregions (2015). The picture is that
of a \emph{dual resource-based economy}: the leading sectors --- mining, oil, steel, and
pulp --- concentrate linkages (the largest pure linkages) but generate little direct
employment and income; extraction operates as an \emph{enclave}, while manufacturing
concentrates the key sectors. A final-demand decomposition quantifies the decoupling: demand
external to the state induces \textbf{61.7\%} of output but only 47.5\% of employment. A
benchmark across the 27 Brazilian states shows that the duality is generic in kind but
extreme in degree --- ES is the second most base-intensive state. Across the territory, a
diversified metropolitan core that retains 90.9\% of its multiplier contrasts with
specialized peripheries that leak; in its openness, linkages flow to the São Paulo--Rio
core with near-zero \emph{feedback} --- the mechanics predicted by the state's size: what
is specific to ES is its composition.

\vspace{0.6em}
\noindent\textbf{Keywords:} input-output; productive structure; multipliers; key sectors;
regional economics; Espírito Santo.
...
estrutura produtiva é atípica: pequena, aberta e organizada em torno de
...
A Seção~\ref{sec:quadro} organiza o quadro analítico desse gênero --- as três lentes da
teoria do desenvolvimento regional e os instrumentos de insumo-produto que as medem ---
e situa seus precedentes estaduais. Para o Espírito Santo, há contribuições \emph{pontuais}:
...
quais seus multiplicadores, setores-chave e polos de crescimento, e \emph{quem puxa}
...
% =====================================================================
\section{Quadro analítico: a teoria regional na linguagem de insumo-produto}
\label{sec:quadro}
Três lentes da teoria do desenvolvimento regional organizam a leitura que este artigo faz
da economia capixaba, e cada uma tem tradução direta num instrumento do método
insumo-produto (Tabela~\ref{tab:quadro}). A primeira é a \emph{base de exportação}: o
crescimento de uma economia regional é comandado pela demanda externa a ela
\citep{north1955}, ao que \citet{tiebout1956} contrapôs o peso das atividades
\emph{residentes}, voltadas ao mercado interno, na ocupação e no multiplicador local.
\citet{richardson1985} formalizou a ponte com o método: o multiplicador de base econômica
é o caso agregado do multiplicador insumo-produto, que o desagrega setor a setor. A lente
é operacionalizada aqui pela decomposição da produção e do emprego pela demanda final ---
que mede diretamente \emph{quem puxa} a economia (\S\ref{sec:retrato}) --- e pelo
fechamento de tipo II, que endogeneiza o circuito induzido residente.
...
\citep{perroux1955}; \citet{hirschman1958} deu ao mecanismo sua métrica --- as ligações
para trás e para frente que induzem investimento a montante e a jusante --- e
\citet{rasmussen1956}, sua normalização. A operacionalização é dupla: os índices de
Rasmussen-Hirschman, neutros ao porte, identificam os \emph{setores-chave}; as ligações
puras \citep{cella1984,guilhoto2005linkages} reintroduzem o porte e identificam os polos
perrouxianos --- grandes \emph{e} articulados (\S\ref{sec:retrato}).

A terceira lente é a da \emph{interdependência centro-periferia}. Na linguagem
inter-regional inaugurada por \citet{isard1951}, o encadeamento de uma região reparte-se
entre o que ela retém e o que transborda (\emph{spillover}); \citet{miller1966} mostrou
que o retorno (\emph{feedback}) é tipicamente pequeno, e \citet{miyazawa1966} decompôs o
mecanismo em multiplicadores internos e externos. Na tradição brasileira, essa mecânica
tem leitura estrutural: o ES como periferia do núcleo industrial paulista \citep{cano1985},
numa hierarquia de núcleos e periferias que se repete em escalas encaixadas
\citep{friedmann1966}. Operacionalizam-na a partição de Isard (vazamento,
\emph{spillover}, \emph{feedback}), a extração hipotética \citep{dietzenbacher1993} e o
modelo nulo de porte (\S\ref{sec:territorio}--\S\ref{sec:plataforma}). As três lentes
são, no fim, três leituras do mesmo sistema $x=(I-A)^{-1}y$: a primeira pergunta quem
move $y$; a segunda, como a inversa propaga o impulso; a terceira, \emph{onde} o
encadeamento se realiza.

\begin{table}[tbp]
\centering\small
\begin{tabular}{@{}p{2.6cm}p{3.4cm}p{4.4cm}p{4.0cm}@{}}
\toprule
Lente teórica & Autores & Instrumento insumo-produto & Onde o artigo mede\\
\midrule
Base de exportação & North (1955); Tiebout (1956); Richardson (1985) &
decomposição pela demanda final; multiplicadores de tipo II &
\S\ref{sec:retrato}: 61,7\% da produção vs.\ 47,5\% do emprego; emprego 27,9$\,\to\,$40,4\\[2pt]
Polos de crescimento & Perroux (1955); Hirschman (1958); Rasmussen (1956) &
índices de Rasmussen-Hirschman; ligações puras (GHS) &
\S\ref{sec:retrato}: setores-chave da transformação; polos mineração 4,4 / metalurgia 3,4\\[2pt]
Centro-periferia & Isard (1951); Miller (1966); Miyazawa (1966); Cano (1985); Friedmann (1966) &
partição inter-regional (\emph{spillover}/\emph{feedback}); extração hipotética; modelo nulo de porte &
\S\ref{sec:territorio}--\S\ref{sec:plataforma}: retenção 90,9\%/66,2\%; \emph{feedback} 0,32\%; $-$13\%\\
\bottomrule
\end{tabular}
\caption{As três lentes da teoria do desenvolvimento regional e sua tradução
insumo-produto neste artigo.}
\label{tab:quadro}
\end{table}

É nesse quadro que se consolidou, no Brasil, o gênero ``estrutura produtiva de
[estado]'': Minas Gerais inaugurou o desenho bi-regional \citep{domingues2002matriz},
seguido de Mato Grosso \citep{figueiredo2004agriculture}, Rio Grande do Sul
\citep{porsse2008matriz}, Pará \citep{sessofilho2010para} --- e, em Santa Catarina, a leitura da indústria
estadual sobre a MIP de 2018 \citep{bittencourt2023sc}; o recorte multiestadual tem precedente no sistema
inter-regional do Nordeste \citep{guilhoto2010nordeste} e o recorte intra-estadual, no
sistema dos municípios paulistas \citep{ichihara2008geo}. Este artigo traz o formato ao
Espírito Santo, acrescido de dois instrumentos de aferição --- o benchmark interestadual
e o modelo nulo de porte --- que separam, no retrato, o que é regularidade do método do
que é específico do estado.
...
Duas fronteiras ficam deliberadamente fora do escopo empírico central. A primeira é a
\textbf{atualização da matriz interestadual brasileira}, já documentada por
\citet{haddad2025matriz} (27 UFs, 68 setores, ano-base 2019, com contas-satélite de emprego,
CO$_2$, energia e água), mas sem reestimação dos indicadores deste artigo; ela é tratada como
referência metodológica, agenda de extensão e checagem contextual pontual. Na Tabela~2 daquela
matriz, o ES combina \emph{déficit} no comércio inter-regional (exportações de R\$\,72,7 bi e
importações de R\$\,88,4 bi) com \emph{superávit} no comércio internacional (R\$\,29,8 bi contra
R\$\,20,8 bi) --- padrão compatível com a leitura de economia aberta/plataforma, mas medido em
outra safra e outra agregação. A segunda é a regionalização da matriz por métodos não-censitários
alternativos: quocientes locacionais corrigidos (FLQ/AFLQ;
\citealp{flegg1995lq,flegg2000flq}), cuja calibragem é sensível ao parâmetro de ajuste
\citep{flegg2016evaluating}; o método CHARM, que corrige o \emph{cross-hauling} que os
quocientes ignoram \citep{tobben2015charm}; as abordagens gravitacionais, que tendem a
reproduzir melhor os fluxos observados \citep{riddington2006comparison}; e o ajuste
biproporcional GRAS \citep{junius2003gras}, central para painéis multi-safra --- sobretudo
com matrizes de entradas negativas ou regiões pequenas. Aqui esses métodos entram apenas
como critério de leitura dos limites --- os fluxos estimados carregam a incerteza da
família não-censitária a que pertencem (\S\ref{sec:limites}) --- e os resultados
reportados são os que se rastreiam diretamente às três matrizes disponíveis.
...
Apêndice~\ref{ap:metodologia}, e os testes de robustez e o tratamento de dados, no
...
restante do Brasil, consumo das famílias, governo/ISFLSF, formação bruta de capital e
variação de estoques. A soma dos componentes reproduz exatamente $x$; a decomposição
identifica \emph{quem puxa} cada setor.
...
método de extração de \citet{cella1984}, que ponderam o encadeamento pela produção do setor. Particionando a economia em
...
microrregiões), mede a reciprocidade do encadeamento; a decomposição multiplicativa de
\citet{miyazawa1966} separa, no mesmo particionamento, multiplicadores internos e externos
(Apêndice~\ref{ap:miyazawa}). Dois instrumentos complementares separam o que é regularidade
de porte do que é estrutura: um \emph{modelo nulo}, que regride cada métrica de abertura no
logaritmo do porte da unidade e lê o resíduo padronizado (escore-$z$;
Apêndice~\ref{ap:nulo}), e a \emph{extração hipotética} \citep{dietzenbacher1993}, que
remove uma unidade do sistema e mede a produção que deixaria de existir sem ela
(Apêndice~\ref{ap:hem}).
...
carrega 25,7\% do emprego, e o governo, com 13,1\%, mais 16,1\%. O contraste setorial
é nítido: refino, metalurgia, mineração e madeira/papel são puxados quase integralmente
de fora (mais de 96\% da produção induzida por demanda externa\footnote{No refino, a
parcela externa excede 100\% (138,6\%) porque a variação de estoques é negativa; a soma
dos componentes reproduz exatamente a produção total (checagem em
\texttt{25\_decomposicao\_fd.py}).}), enquanto administração pública, educação e saúde
dependem em 90\% ou mais da demanda doméstica. A economia de base é, portanto, uma
economia \emph{puxada de fora}: o motor da produção está no resto do país e no exterior,
mas o emprego que esse motor gera fica, em boa parte, aquém do seu peso --- é o mercado
interno que emprega. É a fotografia insumo-produto da \emph{base de exportação}
(\S\ref{sec:quadro}): a demanda externa comanda o crescimento da produção, e a ressalva
tieboutiana reaparece pelo lado do emprego --- são as atividades \emph{residentes},
voltadas ao mercado interno, que carregam a ocupação. O canal tieboutiano
é visível no próprio fechamento do modelo: os multiplicadores de tipo~II --- que
endogeneizam o consumo das famílias capixabas --- elevam o multiplicador médio de emprego de
27,9 para 40,4 empregos por R\$\,1 milhão; é nesse circuito induzido, doméstico, que as
...
\bottomrule
\end{tabular}
\caption{Produção e emprego do ES atribuídos a cada componente da demanda final
(2008), via $x^{(k)}=By^{(k)}$. A demanda externa ao estado (duas primeiras linhas)
puxa 61,7\% da produção, mas 47,5\% do emprego. Fonte: \texttt{25\_decomposicao\_fd.py}
sobre \texttt{outputs/decomposicao\_fd.csv}.}
\label{tab:decomp}
\end{table}
...
São polos na acepção perrouxiana (\S\ref{sec:quadro}): unidades motrizes de grande porte
cujo dinamismo se transmite à economia adjacente pelos encadeamentos --- exatamente o que
a medida de ligações puras quantifica. O grau de integração (coeficiente de variação das
...
todos rotulados; a mineração aparece como forte fornecedora a montante, com baixa ligação
para trás. Fonte: \texttt{17\_caracterizacao\_es\_2008.py} sobre
\texttt{outputs/es\_caracterizacao\_2008.csv}.}
\label{fig:chave}
\end{figure}
...
setores de base (36,4\%, só atrás de Mato Grosso)\footnote{A classificação
base/\emph{commodity} (\texttt{10\_cluster\_setorial.py}) inclui Alimentos entre os setores de
base --- escolha que eleva as parcelas de todos os estados, sobretudo dos cinco do grupo
dinâmico cujo setor dominante é Alimentos (Tabela~\ref{tab:cluster}). O traço capixaba que
independe desse rótulo é o setor \emph{dominante}: o ES é um dos três únicos estados do país,
com Pará e Rio de Janeiro, liderados pela extrativa mineral. A reestimação do ranking
excluindo Alimentos do rótulo fica registrada como checagem de replicação sobre
\texttt{22\_*}/\texttt{10\_*}.} e está entre os de \textbf{menor} multiplicador de
...
dinâmico --- sub-cobertos pelo mercado de capitais, excluído o núcleo SP/RJ ---, o ES é o mais
...
\bottomrule
\end{tabular}
\caption{O ES e os estados de crescimento dinâmico \emph{versus} o núcleo SP/RJ. O ES é o
2\textordmasculine\ mais intensivo em setores de base do país e, com o Pará e o Rio de Janeiro,
um dos três do país cujo setor dominante é a Mineração. Fonte:
...
de petróleo e de minério abaixo (enclaves de baixa ligação doméstica). Fonte:
\texttt{18\_trajetoria\_n68.py} sobre \texttt{outputs/trajetoria\_n68.csv}.}
\label{fig:traj}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Multiplicadores de produção e renda (tipo I, ponderados pelo VBP setorial) e
retenção intra-territorial do multiplicador, por microrregião (2015). A metrópole retém;
a periferia extrativa vaza. Nota: a retenção é a razão das somas ponderadas
...
(Metropolitana: vazamento de 8,4\%, i.e., retenção de 91,6\%); as duas agregações descrevem
o mesmo fluxo. Fonte: \texttt{24\_micro\_mult\_chave.py} sobre
\texttt{outputs/micro\_multiplicadores.csv}.}
\label{tab:micro}
\end{table}
...
não-metálicos) pela geração de emprego e renda; o quociente locacional e a retenção aqui
adicionam a esse retrato as dimensões da \emph{especialização} e do \emph{fluxo} --- não
apenas onde os multiplicadores são altos, mas em que cada território é especializado e
quanto do encadeamento fica nele ---, e os dois resultados
...
especialização extrema. Exibem-se os dez setores (de 35) que definem as vocações
territoriais; a escala de cor é truncada em $\mathrm{LQ}=6$ --- os valores anotados são
os exatos. Fonte: \texttt{20\_caracterizacao\_micro.py} sobre \texttt{outputs/micro\_LQ.csv}.}
\label{fig:vocacao}
\end{figure}
...
multiplicador de produção. Vazamento médio: simples 24,9\%; ponderado pela produção 22,9\%.
Fonte: \texttt{12\_tab\_setores.py} sobre \texttt{outputs/es\_setores\_resultados.csv}.}
\label{tab:setores}
\end{table}
...
\textbf{0,32\%} (Figura~\ref{fig:sankey}). A interdependência é quase unilateral. E o destino
é concentrado: o \textbf{Sudeste} (excluído o ES) absorve \textbf{65,4\%} do \emph{spillover}
--- São Paulo 37,8\%, Rio de Janeiro 14,7\% e Minas Gerais 12,9\% --- e o núcleo São
Paulo--Rio de Janeiro, \textbf{52,5\%}. O ES alimenta seus vizinhos mais ricos; o encadeamento
que escapa é capturado a jusante e quase não retorna. O padrão não é exclusividade capixaba
entre os estados \emph{commodity}-exportadores: em Mato Grosso, a soja gera 8 empregos diretos
por R\$\,1 milhão de demanda final, ante 31 indiretos \citep{figueiredo2004agriculture}; e, no
agronegócio gaúcho, \citet{peixoto2013metodologia} estimam que apenas 0,4\% do valor adicionado
do restante do Brasil decorre de elos com o Rio Grande do Sul, ante 7,9\% no sentido inverso ---
a mesma assimetria de captura a jusante que o ES exibe pelo lado do minério.

\begin{figure}[tbp]
\centering
\includegraphics[width=.92\linewidth]{es_sankey.pdf}
\caption{Destino do \emph{spillover} do ES, estratificado por macrorregião e estado
(injeção da demanda final do ES por produtos do ES, matriz interestadual de 2008): dos
R\$\,18,1 bilhões transbordados, o Sudeste absorve 65\% e o núcleo São Paulo--Rio de
Janeiro, 52,5\%; o \emph{feedback} de volta ao ES é de apenas 0,32\% da injeção. No
sistema bi-regional do texto, o mesmo experimento transborda R\$\,18,2 bi --- a diferença
é de agregação (Apêndice~\ref{ap:medias}). Fonte: \texttt{02\_interestadual.py} sobre
\texttt{outputs/es\_spillover\_destino.csv}.}
\label{fig:sankey}
\end{figure}
...
vaza-da-periferia configura, pois, a relação \emph{centro-periferia} do quadro analítico
(\S\ref{sec:quadro}), reproduzida em \emph{escalas aninhadas}: a periferia está para o ES
como o ES está para o Sudeste --- dentro do estado, o mesmo desenho que a tradição
regional brasileira lê entre o ES e o núcleo industrial paulista. É a mesma assinatura de
\emph{enclave} que a concentração territorial já indicava (\S\ref{sec:territorio}), agora
pelo lado do fluxo. A pequenez do \emph{feedback} é, em boa medida, regularidade de porte
(\S\ref{sec:quadro}) --- e o modelo nulo (Apêndice~\ref{ap:nulo}) a quantifica: regredidas as três métricas de
abertura no logaritmo do porte das 27 UFs, o ES não é \emph{outlier} em nenhuma
(escores-$z$ de $+0{,}45$ no vazamento, $-0{,}05$ na razão de \emph{feedback} e $+0{,}03$
na concentração de destino; Tabela~\ref{tab:zscores}), e seu \emph{feedback} observado
(0,32\%) fica até \emph{abaixo} do previsto para o seu porte (0,45\%). O retorno é, antes,
privilégio de núcleo: a razão de \emph{feedback} média de SP+RJ (1,97\%) é quatro vezes a
dos oito estados de crescimento dinâmico (0,49\%). Nem a concentração do destino singulariza
o ES --- Minas Gerais (60,0\%) e Rio Grande do Sul (55,2\%) escoam ao núcleo parcela
\emph{maior} que a capixaba (52,5\%; 14\textordmasculine\ entre as 27 UFs no HHI de
destino). O que é próprio do ES, como mostra o benchmark composicional
(Tabela~\ref{tab:bench}), não é a mecânica do vazamento, mas a \emph{composição de base}
que essa mecânica genérica transporta.

\begin{figure}[tbp]
\centering
\includegraphics[width=\linewidth]{fig_fractal.pdf}
...
mesma mecânica de posição-e-porte opera em ambas. Fonte: \texttt{14\_benchmark\_ufs.py} e
\texttt{15\_intra\_es\_fractal.py} sobre \texttt{outputs/benchmark\_ufs.csv} e
\texttt{outputs/intra\_es\_fractal.csv}.}
\label{fig:fractal2}
\end{figure}

\paragraph{Posição em cadeias de valor.} A abertura tem uma contraparte na cadeia: o ES vaza
encadeamento porque está \emph{a montante}. Medida a \emph{upstreamness} de Antràs-Chor
\citep{antras2012measuring} sobre a base internacional do WIOD \citep{timmer2015wiod}, a pauta
exportadora capixaba tem posição ponderada de \textbf{3,12}, contra 1,91 do Brasil e 2,31 da
média mundial, com a mineração entre as atividades mais a montante do mundo
($U=3{,}82$; 33\textordmasculine\ de 2.279 setores válidos, percentil~98; Figura~\ref{fig:upstream}).
O estado supre insumos primários e intermediários cuja captura de valor a jusante se realiza em
outras regiões e países --- a leitura por \emph{ranking}, e não por nível bruto, segue
\citet{araujojunior2018cgv}, com o tratamento da base detalhado no
Apêndice~\ref{ap:wiod}. É a tradução, na escala da cadeia global, do mesmo diagnóstico:
uma economia de base, fornecedora a montante.

\begin{figure}[tbp]
\centering
\includegraphics[width=.86\linewidth]{es_upstreamness.pdf}
\caption{\emph{Upstreamness} (Antràs-Chor) da pauta exportadora do ES, WIOD 2014: o estado
posiciona-se a montante da cadeia de valor (pauta 3,12; Brasil 1,91; mundo 2,31). Barras
ordenadas pelo peso do setor na pauta exportadora capixaba. Fonte:
\texttt{08\_es\_upstreamness.py} sobre \texttt{outputs/es\_upstreamness.csv}.}
\label{fig:upstream}
\end{figure}
...
Base exportadora que comanda a produção, polos motrizes que concentram o encadeamento e
hierarquia centro-periferia que captura o valor são, aqui, três descrições do mesmo dado
--- as três lentes do quadro analítico (\S\ref{sec:quadro}) convergindo sobre o mesmo
sistema.
...
valor à celulose, às rochas, ao agro). Três alavancas emergem do próprio dado ---
condicionais, como toda a leitura de política aqui, às safras das âncoras
(\S\ref{sec:limites}). \textbf{(i) Adensar a jusante da base mineral}: a
...
serranas; a extração hipotética dá a medida do risco que essa agenda mitiga --- sem a metrópole,
a produção da periferia cairia 13,0\%, e diversificar os territórios é também reduzir a
mono-dependência do núcleo. Por fim, a forte presença de setores extrativos de alto valor e baixo emprego direto ---
...
diversificação é, ela própria, uma política estrutural --- tanto mais porque, como mostra o
modelo nulo, o retorno via encadeamento é estruturalmente limitado para o porte capixaba (o
\emph{feedback} observado fica abaixo até do previsto): a captura fiscal é o canal de retorno
que resta.

Fica registrada, por fim, a extensão \emph{ambiental} do diagnóstico: assim como exporta valor a
montante, a economia de base capixaba exporta \emph{carbono incorporado} --- minério, siderurgia,
refino e celulose são intensivos em CO$_2$. Estimar esse \emph{carbono-plataforma} (multiplicador de
emissão, vazamento de carbono e pegada da pauta) é a agenda imediata, viável com o satélite de CO$_2$
da matriz interestadual de 2019 \citep{haddad2025matriz}, na linha de \citet{carvalho2009avaliacao};
o aparato reprodutível já está montado, à espera do vetor de emissões.
...
vetor de emprego (apenas remunerações). Como em toda a família não-censitária, os fluxos
origem-destino são aproximações --- métodos gravitacionais reproduzem melhor os fluxos
...
neutros ao porte. \textbf{(v)} A leitura de ``economia-plataforma'' é uma interpretação
estrutural derivada dos multiplicadores, do \emph{spillover}/\emph{feedback} e do benchmark de
porte; não deve ser lida como tipologia causal universal. A auditoria desta versão separou os
\emph{claims} diretamente derivados das matrizes, os \emph{proxies} nacionais e as implicações de
política, mantendo estas últimas como inferências condicionais.
...
nas rotinas reprodutíveis de \texttt{pesquisa/}. Em todo o apêndice supõe-se $A\ge 0$
\emph{produtiva} (condição de Hawkins--Simon; raio espectral menor que a unidade), o que
garante que as inversas de Leontief utilizadas existem e são não-negativas --- condição
...
\label{ap:puras}
Na linhagem do método de extração de \citet{cella1984}, as ligações puras
\citep{guilhoto2005linkages} particionam $A$ entre o setor $j$ e o resto $r$,
\begin{equation}
...
economia sobre o setor $j$ ($\mathrm{PFL}_j$ já é escalar, pois $A_{jr}$ é vetor-linha). No texto, $\mathrm{PBL}/\mathrm{PFL}/\mathrm{PTL}$ são
reportadas padronizadas pela média dos 26 setores do ES.
...
\subsection{Modelo nulo de porte}
\label{ap:nulo}
Para separar o que é regularidade de tamanho do que é estrutura, cada métrica de abertura
$m_r$ (vazamento, razão de \emph{feedback}, HHI de destino) é regredida no logaritmo do
porte da unidade $r$:
\begin{equation}
m_r=\alpha+\beta\ln(x_r)+\varepsilon_r,
\label{eq:nulo}
\end{equation}
em que $x_r$ é a produção total da unidade. O $R^2$ mede quanto o porte explica; o resíduo
padronizado (escore-$z$) $z_r=\hat{\varepsilon}_r/\hat{\sigma}_\varepsilon$ mede o quanto a
unidade-foco excede o previsto pelo porte --- a candidata a especificidade. O ajuste é
\emph{descritivo}: com $n=27$ (e $n=10$ na escala microrregional) não se pretende
inferência estatística, apenas a separação contábil entre o previsto pelo porte e o
resíduo. Os resultados para o ES estão no Apêndice~\ref{ap:zscores}.
...
\subsection{Tratamento da base WIOD (2014)}
\label{ap:wiod}
A demanda final implícita da WIOD incorpora variação de estoques e discrepância
estatística, o que produz \textbf{49} dos 2.464 pares país-setor com demanda final
implícita $\le 0$ e uma cauda de \emph{upstreamness} sem sentido econômico ($U>10$). Como
entradas negativas de bases MRIO devem ser reconciliadas, e não interpretadas como fluxo
econômico, excluem-se os setores degenerados e a cauda implausível --- \textbf{185} pares
removidos, \textbf{2.279} válidos --- e a posição setorial é reportada por \emph{nível e
ranking} (mineração $U=3{,}82$; 33\textordmasculine\ de 2.279), não pelo percentil da
distribuição bruta. As médias ponderadas (pauta ES 3,12; Brasil 1,91; mundo 2,31) são
insensíveis a esse tratamento.
...
Metropolitana passa de 8,4\% para 7,9\% e o $R^2$ do modelo nulo intra-estadual permanece
em 0,91: nenhum resultado territorial reportado é dirigido por esse setor.
...
\subsection{Escores-$z$ do modelo nulo (27 UFs, 2008)}
\label{ap:zscores}
A Tabela~\ref{tab:zscores} consolida o modelo nulo da eq.~\eqref{eq:nulo} para o ES. Em
nenhuma das três métricas de decomposição o estado é \emph{outlier}; a especificidade
capixaba é composicional (Tabela~\ref{tab:bench}), não mecânica.
...
\midrule
Vazamento do multiplicador & 22,8\% & 21,3\% & $+1{,}6$ p.p. & $+0{,}45$ & 8\textordmasculine/27 \\
Razão de \emph{feedback} & 0,32\% & 0,45\% & $-0{,}12$ p.p. & $-0{,}05$ & 8\textordmasculine/27 \\
HHI de destino do \emph{spillover} & 0,193 & 0,193 & $+0{,}000$ & $+0{,}03$ & 14\textordmasculine/27 \\
\bottomrule
\end{tabular}
\caption{Modelo nulo $m_r=\alpha+\beta\ln(x_r)+\varepsilon_r$ nas 27 UFs (2008): o porte
explica $R^2=0{,}59$ da razão de \emph{feedback} (e $R^2=0{,}91$ na escala das dez
microrregiões), mas quase nada do vazamento e da concentração de destino
($R^2\approx0{,}01$). O ES não excede o previsto em nenhuma métrica. Fonte:
...
production and trade flows. \textit{American Economic Review}, 102(3), 412--416.

\bibitem[Araújo Júnior(2018)]{araujojunior2018cgv}
Araújo Júnior, I. F. de (2018). \textit{Três ensaios sobre a análise das cadeias globais de
valor}. Tese de Doutorado, Universidade Federal de Juiz de Fora.

\bibitem[Bittencourt et al.(2023)]{bittencourt2023sc}
Bittencourt, P., Fachinello, A. L., Mazzucco, B., \& Arroyo, A. F. (2023). A indústria
catarinense entre seu passado agro e o futuro TEC: a integração local, nacional e global
a partir da MIP 2018. \textit{Revista Brasileira de Estudos Regionais e Urbanos}, 17(1),
90--112.

\bibitem[Cano(1985)]{cano1985}
Cano, W. (1985). \textit{Desequilíbrios regionais e concentração industrial no Brasil:
1930--1970}. São Paulo: Global Editora.

\bibitem[Carvalho \& Perobelli(2009)]{carvalho2009avaliacao}
Carvalho, T. S., \& Perobelli, F. S. (2009). Avaliação da intensidade de emissões de CO$_2$
setoriais e na estrutura de exportações: um modelo inter-regional de insumo-produto São
Paulo/restante do Brasil. \textit{Economia Aplicada}, 13(1), 99--124.

\bibitem[Cella(1984)]{cella1984}
Cella, G. (1984). The input-output measurement of interindustry linkages.
\textit{Oxford Bulletin of Economics and Statistics}, 46(1), 73--84.
...
method: EC input-output comparisons. \textit{Economic Systems Research}, 5(2), 185--206.
...
Flegg, A. T., Webber, C. D., \& Elliott, M. V. (1995). On the appropriate use of location
quotients in generating regional input-output tables. \textit{Regional Studies}, 29(6), 547--561.

\bibitem[Flegg \& Webber(2000)]{flegg2000flq}
Flegg, A. T., \& Webber, C. D. (2000). Regional size, regional specialization and the FLQ
formula. \textit{Regional Studies}, 34(6), 563--569.
...
Guilhoto, J. J. M., Sonis, M., \& Hewings, G. J. D. (2005). Linkages and multipliers in a
multiregional framework: integration of alternative approaches. \textit{Australasian
Journal of Regional Studies}, 11(1), 75--89.

\bibitem[Guilhoto et al.(2010)]{guilhoto2010nordeste}
Guilhoto, J. J. M., Azzoni, C. R., Ichihara, S. M., Kadota, D. K., \& Haddad, E. A.
(2010). \textit{Matriz de insumo-produto do Nordeste e estados: metodologia e
resultados}. Fortaleza: Banco do Nordeste do Brasil.
...
Haddad, E. A., Araújo, I. F. de, Rocha, A., \& Vale, V. de A. (2025). Matriz interestadual de
insumo-produto para o Brasil, 2019. \textit{Revista Brasileira de Estudos Regionais e Urbanos},
19(4), 607--638.
...
\bibitem[Ichihara \& Guilhoto(2008)]{ichihara2008geo}
Ichihara, S. M., \& Guilhoto, J. J. M. (2008). Geoprocessing and estimation of
interregional input-output systems: an application to the state of São Paulo in Brazil.
In: \textit{48th Congress of the European Regional Science Association}. Liverpool: ERSA.
...
\bibitem[Junius \& Oosterhaven(2003)]{junius2003gras}
Junius, T., \& Oosterhaven, J. (2003). The solution of updating or regionalizing a matrix with
both positive and negative entries. \textit{Economic Systems Research}, 15(1), 87--96.
...
\textit{Hitotsubashi Journal of Economics}, 7(1), 38--55.
...
North, D. C. (1955). Location theory and regional economic growth. \textit{Journal of
...
Perroux, F. (1955). Note sur la notion de ``pôle de croissance''. \textit{Économie
Appliquée}, 8(1--2), 307--320.
...
Richardson, H. W. (1985). Input-output and economic base multipliers: looking backward
and forward. \textit{Journal of Regional Science}, 25(4), 607--661.
...
\bibitem[Sesso Filho \& Guilhoto(2010)]{sessofilho2010para}
Sesso Filho, U. A., \& Guilhoto, J. J. M. (2010). Estrutura produtiva do Pará e sua
inter-relação com o restante da economia brasileira: uma análise de insumo-produto.
\textit{Revista Brasileira de Estudos Regionais e Urbanos}, 4(2), 81--104.

\bibitem[Tiebout(1956)]{tiebout1956}
Tiebout, C. M. (1956). Exports and regional economic growth. \textit{Journal of Political
...
user guide to the World Input-Output Database. \textit{Review of International Economics}, 23(3),
575--605.
